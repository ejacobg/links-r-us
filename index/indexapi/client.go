...
func (c *TextIndexerClient) Search(query index.Query) (index.Iterator, error) {
	...
	if err != nil {
		...
		cancelFn()
		return nil, fmt.Errorf("expected server to report the result count before sending any documents")
	}
...
func (it *resultIterator) Next() bool {
	...
	if resDoc == nil {
		it.cancelFn()
		it.lastErr = fmt.Errorf("received nil document in search result list")
		return false
	}
