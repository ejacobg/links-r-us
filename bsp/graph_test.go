...
func (s *GraphTestSuite) TestMessageExchange(c *gc.C) {
	g, err := bsp.NewGraph(bsp.GraphConfig{
		ComputeFn: func(g *bsp.Graph, v *bsp.Vertex, msgIt bsp.Iterator) error {
			v.Freeze()
			if g.Superstep() == 0 {
				var dst string
				switch v.ID() {
...
func (s *GraphTestSuite) TestAggregator(c *gc.C) {
	...
	offset := 5
	g.RegisterAggregator("counter", new(bsp.IntAccumulator))
...
func (s *GraphTestSuite) TestMessageRelay(c *gc.C) {
	...
	ex1 := bsp.NewExecutor(g1, bsp.ExecutorCallbacks{
		PreStep: func(context.Context, *bsp.Graph) error {
			syncCh <- struct{}{}
			return nil
		},
		PostStep: func(context.Context, *bsp.Graph, int) error {
			syncCh <- struct{}{}
			return nil
		},
	})
	ex2 := bsp.NewExecutor(g2, bsp.ExecutorCallbacks{
		PreStep: func(context.Context, *bsp.Graph) error {
			<-syncCh
			return nil
		},
...
func (r localRelayer) Relay(dstID string, msg bsp.Message) error {
	if r.relayErr != nil {
		return r.relayErr
	}
...
func execFixedSteps(g *bsp.Graph, numSteps int) error {
