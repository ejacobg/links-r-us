...
func (c *Calculator) Executor() *bsp.Executor {
	...
	cb := bsp.ExecutorCallbacks{
		...
		PostStepKeepRunning: func(_ context.Context, g *bsp.Graph, _ int) (bool, error) {
...
func (c *Calculator) registerAggregators() {
	c.g.RegisterAggregator("page_count", new(bsp.IntAccumulator))
	c.g.RegisterAggregator("residual_0", new(bsp.Float64Accumulator))
	c.g.RegisterAggregator("residual_1", new(bsp.Float64Accumulator))
	c.g.RegisterAggregator("SAD", new(bsp.Float64Accumulator))
}
